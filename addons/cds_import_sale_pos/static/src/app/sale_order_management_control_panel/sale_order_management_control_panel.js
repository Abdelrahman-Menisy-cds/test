...
export class SaleOrderManagementControlPanel extends Component {
    static template = "cds_import_sale_pos.SaleOrderManagementControlPanel";
    ...
    setup() {
        ...
        const currentPartner = this.pos.getOrder()?.getPartner();
        if (currentPartner) {
     ...
     */
    _computeDomain() {
        // Parse the dynamic domain from pos.config.sale_domain
    // Parse the dynamic domain from pos.config.sale_domain
        let domain = [];
        try {
            const configDomain = this.pos.config.sale_domain || '[]';
            // Use Domain class to parse Python string representation
            const domainObj = new Domain(configDomain);
            domain = domainObj.toList({});
        } catch (e) {
            console.error('Error parsing sale_domain:', e);
