...
class StockCardReport(models.TransientModel):
    ...
    def _compute_results(self):
        ...
        self._cr.execute(
            ...
            SELECT move.date, move.product_id, move.product_qty,
                move.quantity, move.product_uom, move.reference,
    ...
    def _get_html(self):
        ...
        if report:
            rcontext["o"] = report
            template = "stock_card_report.report_stock_card_report_html"
            result["html"] = self.env.ref(template)._render_template(template, rcontext)
        return result
