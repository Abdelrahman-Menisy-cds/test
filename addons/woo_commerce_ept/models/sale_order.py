...
class SaleOrder(models.Model):
    ...
    def _search_woo_order_ids(self, operator, value):
        ...
        if operator == 'in':
            query = """
            SELECT so.id  FROM stock_picking sp INNER JOIN stock_move sm ON sm.picking_id = sp.id
                        INNER JOIN stock_reference_move_rel smr ON smr.move_id = sm.id
                        INNER JOIN stock_reference_sale_rel srsr ON srsr.reference_id = smr.reference_id
                        INNER JOIN sale_order so ON so.id = srsr.sale_id                 
                        INNER JOIN stock_location ON stock_location.id = sp.location_dest_id AND stock_location.usage = 'customer'
                        ...
                        WHERE sm.picking_id IS NULL AND sm.state = 'done' AND so.woo_instance_id IS NOT NULL
            """
    ...
    _woo_sale_order_unique_constraint = models.Constraint('unique(woo_order_id,woo_instance_id,woo_order_number)',
    ...
    def create_woo_order_line(self, line_id, product, quantity, price, taxes, tax_included, woo_instance,
        ...
        if woo_instance.apply_tax == "create_woo_tax":
            ...
            woo_so_line_vals.update({"tax_ids": [(6, 0, tax_ids)]})
