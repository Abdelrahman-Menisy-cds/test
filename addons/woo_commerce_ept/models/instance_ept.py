...
class WooInstanceEpt(models.Model):
    ...
    def get_compare_data(self, record):
        ...
        def get_compared_week_data(record):
            ...
            qry = """select sum(amount_untaxed) as previous_week from sale_order
                                ...
                                and woo_instance_id=%s and state in ('sale','done')
                                """
            self.env.cr.execute(qry, (day_of_week, record.id))
        ...
        def get_compared_month_data(record):
            ...
            qry = """select sum(amount_untaxed) as previous_month from sale_order where date(date_order)
                                ...
                                and woo_instance_id=%s and state in ('sale','done')
                                """
            self.env.cr.execute(qry, (day_of_month, record.id,))
        ...
        def get_compared_year_data(record):
            ...
            qry = """select sum(amount_untaxed) as previous_year from sale_order where date(date_order)
                                ...
                                and woo_instance_id=%s and state in ('sale','done')
                                """
            self.env.cr.execute(qry, (delta, record.id,))
            previous_data = self.env.cr.dictfetchone()
            if previous_data:
    ...
    def get_shipped_orders(self):
        ...
        shipped_query = """
            ...
            WHERE sp.updated_in_woo = TRUE
              AND sp.state != 'cancel'
              AND so.woo_instance_id = %s
        """
    ...
    _unique_host = models.Constraint('unique(woo_host)',
        ...
        if operation_type == "import":
            ...
            for meta_value in meta_vals_list:
                meta_mapping = self.meta_mapping_ids.filtered(
                    lambda mapping: mapping.woo_operation == self.env.context.get(
