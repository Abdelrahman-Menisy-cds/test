...
class WooProcessImportExport(models.TransientModel):
    ...
    def woo_export_products(self):
        ...
        woo_template_ids = self.env.context.get('active_ids')
    ...
    def update_products(self):
        ...
        woo_tmpl_ids = self.env.context.get('active_ids')
    ...
    def update_export_category_tags_coupons_in_woo(self):
        ...
        Migrated by Maulik Barad on Date 07-Oct-2021.
        """
        process_type = self.env.context.get("process", "")
    ...
    def export_tags_in_woo(self):
        ...
        if self.env.context.get("process", "") == "export_tags":
            tags_need_to_export = woo_tags_obj.search(
                [("id", "in", self.env.context.get("active_ids")), ("exported_in_woo", "=", False)])
        else:
            tags_need_to_export = woo_tags_obj.search(
    ...
    def update_tags_in_woo(self):
        ...
        """
        woo_tags_obj = self.env["woo.tags.ept"]
        if self.env.context.get("process", "") == "update_tags":
            tags_need_to_export = woo_tags_obj.search(
                [("id", "in", self.env.context.get("active_ids")), ("exported_in_woo", "=", True)])
        else:
            tags_need_to_export = woo_tags_obj.search(
    ...
    def update_product_categ(self):
        ...
        woo_categ_ids = self.env.context.get('active_ids')

        if woo_categ_ids and self.env.context.get('process'):
    ...
    def export_woo_coupons(self):
        ...
        if coupons_ids and self.env.context.get('process'):
    ...
    def update_woo_coupons(self):
        ...
        coupon_ids = self.env.context.get('active_ids')
        if coupon_ids and self.env.context.get('process'):
