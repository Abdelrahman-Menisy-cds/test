...
class WooManualQueueProcessEpt(models.TransientModel):
    ...
    def process_queue_manually(self):
        ...
        """
        queue_type = self.env.context.get("queue_type", "")
        if queue_type == "order":
    ...
    def process_order_queue_manually(self):
        ...
        order_queue_ids = order_data_queue_obj.browse(self.env.context.get('active_ids')).filtered(
    ...
    def process_customer_queue_manually(self):
        ...
        customer_queues = customer_data_queue_obj.browse(
            self.env.context.get('active_ids', False)).filtered(lambda x: x.state != "done")
    ...
    def process_coupon_queue_manually(self):
        ...
        queue_line_active_ids = []
        coupon_queue_ids = coupon_data_queue_obj.browse(self.env.context.get('active_ids')).filtered(
            lambda x: x.state != 'done')
    ...
    def process_export_stock_queue_manually(self):
        ...
        queue_line_active_ids = []
        stock_queues = export_stock_queue_obj.browse(self.env.context.get('active_ids')).filtered(
            lambda x: x.state != "done")
